...
流是从支持数据处理操作的源生成的字节序列，源可以是数组、文件、集合、函数。它不是数据结构不保存数据，主要目的在于计算。

Stream中的操作可以分为两大类：**中间操作**（Intermediate operations）与**结束操作**（Terminal operations），中间操作只是对操作进行了记录，只有结束操作才会触发实际的计算（即惰性求值）

采用了流水线的方式，Stream中的每个操作都对应一个Pipeline对象，最终整个Stream操作会转化为一个Pipeline的双向链表。直到结束操作，结束操作会使用递归的方式不断调用onWrapSink函数，由下向上构建一个Sink链，然后执行流操作。
